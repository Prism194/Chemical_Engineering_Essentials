...
2\. 결론
- $S_{G}\ge0$ 이므로, $W_{lost}\ge0$이다. 만약 공정이 완전한 가역 공정이면 손실된 일은 0일 것이다.
- 의의 : 공정의 비가역성이 커질수록 엔트로피의 생성 속도와 일로 전환될 수 없는 에너지의 양이 커진다. 효율적인 공저을 위해서는 엔트로피 생성을 최소화해야 한다.

5.9 열역학 제 3법칙
1\. 유도 과정
- 같은 화학물질이나 다른 결정형태를 가지는 경우, 무질서한 형태의 엔트로피가 결정형태의 엔트로피보다 더 크게 나타난다
- 그러나 0K에서의 엔트로피는 모든 형태들에 대해서 그 값이 동일하다
2\. 열역학 제 3법칙
<font color="#003380"><strong><center>절대 엔트로피는 절대온도 0도의 모든 완전한 결정형 물질들에 대해 0이라 가정할 수 있다</center></strong></font>
3\. 열역학 제 3법칙의 적용
- 어떤 물질의 절대 엔트로피를 구할 수 있다
- 그러나 열역학적 분석에서는 상대적인 값들이 주로 요구되며, 0K에서의 완벽한 결정상태보다는 일부 기준상태를 사용한다

5.10 미시적 관점에 따른 엔트로피
1\. 무질서함 정량화
- 무질서한 상태의 증가는 엔트로피 증가라 볼 수 있다
- Boltzmann의 엔트로피 식 : $S=k\ln\Omega$, $\Omega$ : 미시적인 입자들이 접근 가능한 상태들 사이에 분포될 수 있는 서로 다른 방법들의 수
